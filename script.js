...
// Position the desktop flyer directly under the sticky note (sticky1).
function positionFlyerUnderSticky() {
  const sticky = document.getElementById('sticky1');
  const flyer = document.getElementById('desktopFlyer');
  if (!sticky || !flyer) return;

  const rect = sticky.getBoundingClientRect();
  // Compute coordinates relative to the document
  const left = Math.max(8, Math.round(rect.left + window.scrollX));
  const top = Math.round(rect.bottom + window.scrollY + 30); // 12px gap under sticky

  flyer.style.position = 'absolute';
  flyer.style.left = left + 'px';
  flyer.style.top = top + 'px';
  // clear any right-based placement
  flyer.style.right = '';
}

// Run on load and resize so flyer stays under sticky note
window.addEventListener('load', positionFlyerUnderSticky);
window.addEventListener('resize', () => setTimeout(positionFlyerUnderSticky, 80));
